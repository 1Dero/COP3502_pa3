...
// Postcondition: Returns the number of permutations 
...
int permutations(int perm[MAX_PEOPLE], int used[MAX_PEOPLE], int k);
...
int permutations(int perm[MAX_PEOPLE], int used[MAX_PEOPLE], int k) {
    ...
    for(int i = 0; i<n; i++) {
        if(!used[i]) {
            used[i] = 1;
            perm[k] = i;
            count += permutations(perm, used, k+1);
            used[i] = 0;
        }
    }
    return count;
